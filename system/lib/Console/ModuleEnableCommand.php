...
class ModuleEnableCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach($modules as $module)
        {
            ...
            {
                $output->writeln('<warning>Module <b-warning>' . $module . '</b-warning> is already enabled.</warning>');
                continue;
            }
            
            if(Module::enable($module))
            {
                $output->writeln('<info>Module <b-info>' . $module . '</b-info> was enabled.</info>');
            }
            else
            {
                $output->writeln('<error>Module <b-error>' . $module . '</b-error> could not be enabled.</error>');
            }
